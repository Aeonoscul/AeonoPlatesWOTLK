...
--//Настройки загружаются из AeonoPlatesDB.cfg (SavedVariables)
local cfg = AeonoPlatesDB.cfg or {
...
--// GUI Integration //--
-- This function is called from the GUI when settings change
function AeonoPlates_Update()
    -- Settings are already applied to cfg via AeonoPlatesDB.cfg
    -- The OnUpdate loop will pick up changes when frames are recreated
    -- For immediate update, we could force a refresh here if needed
end 